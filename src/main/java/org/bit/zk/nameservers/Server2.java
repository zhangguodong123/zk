...
public class Server2 implements Watcher{
	private static final String address="192.168.56.92";
	...
	public void initZookeeper(){
		try {
			...
			System.out.println("the server2 "+Constrant.SUB_PATH+" is create successfuly.path:"+subcreate);
	...
	public static void shutdownZooKeeper(ZooKeeper zk){
		 Runtime.getRuntime().addShutdownHook(new Thread() {
	            public void run() {
	            	try {
	        			zk.close();
	        			System.out.println("关闭 Server2 Zookeeper");
	        		} catch (InterruptedException e) {
	        			e.printStackTrace();
	        		}
	            }
